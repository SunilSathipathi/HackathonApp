...
class DynamicAIQueryService:
    ...
    def _generate_sql(self, question: str, schema_summary: str) -> Dict[str, Any]:
        system = (
            ...
            "Goal title filtering: If the question mentions a specific goal name or keyword (e.g., 'jenkins goal' or a quoted title), include a filter on g.title ILIKE :goal_title using a wildcard parameter (e.g., '%jenkins%').\n"
            "Example for 'who assigned <goal> to <person>': SELECT g.goal_id, e_by.full_name AS assigned_by, e_to.full_name AS assigned_to FROM goals g JOIN employees e_by ON e_by.employee_id = g.assigned_by_employee_id JOIN employees e_to ON e_to.employee_id = g.assigned_to_employee_id WHERE e_to.full_name ILIKE :employee_name AND g.title ILIKE :goal_title LIMIT 50.\n"
            "Projects assignment mapping: For 'who is assigned to <project>' or '<project> is assigned to whom', join employee_projects ep -> projects p by ep.project_id = p.project_id, then join employees e ON e.employee_id = ep.employee_id. Filter p.name ILIKE :project_name. SELECT e.full_name, e.employee_id, ep.role.\n"
            "Project manager mapping: If asking 'who manages <project>' or '<project> project manager', prefer p.manager_employee_id joining to employees ON employees.employee_id = p.manager_employee_id, or use p.project_manager text when employee_id is missing.\n"
